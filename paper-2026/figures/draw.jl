	...
	for (Re, c) in zip(Reynolds, colormaps)
		if αβs[Re][2] > 0 && αβs[Re][1] > 0
			...
			for n in 1:10
				...
				lines!(axb, l ./ (2π*leq), Γⁿ .* α ^ (n / 2); color = c)
				scatter!(axb, l ./ (2π*leq), Γⁿ .* α ^ (n / 2); color = :white, strokecolor = c, strokewidth = 0.5, markersize = 3, label = "$(Re)")

				lines!(axc, l ./ (2π*leq), Γⁿ .* β ^ (n / 2) .* l .^ (-n); color = c)
				scatter!(axc, l ./ (2π*leq), Γⁿ .* β ^ (n / 2) .* l .^ (-n); color = :white, strokecolor = c, strokewidth = 0.5, markersize = 3, label = "$(Re)")

				lines!(axd, l ./ (2π*leq), Γⁿ .* (α * β)^(n / 4) .* l .^ (-n/2); color = c)
				scatter!(axd, l ./ (2π*leq), Γⁿ .* (α * β) ^ (n / 4) .* l .^ (-n/2); color = :white, strokecolor = c, strokewidth = 0.5, markersize = 3, label = "$(Re)")
			end
		end
	end
