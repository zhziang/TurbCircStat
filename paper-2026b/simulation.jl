using Random, HDF5, ArgParse, CUDA
...
x₀, y₀ = rand(npoints), rand(npoints)

Γ = CuArray(Γ)

x₀, y₀ = CuArray(x₀), CuArray(y₀) 

initial_hamiltonian = hamiltonian(x₀, y₀, Γ)
...
prob = ODE.ODEProblem(odefunc!, vcat(x₀, y₀), (0, tspan))

sol = ODE.solve(prob, ODE.Vern7(); dt = dt, callback = mproj, progress = true, save_everystep = false)
...
if SMLB.successful_retcode(sol)
	...
	h5open(output_path, "w") do fid
		write_dataset(fid, "pv Positions", hcat(sol.u[sol.t .≥ tspan/10]...))
